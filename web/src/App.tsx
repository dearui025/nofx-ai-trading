...
import { EnvironmentStatusPage } from './components/EnvironmentStatusPage';
import RiskControlV2Page from './components/RiskControlV2Page';
import FrequencyManager from './components/FrequencyManager';
import AIOptimizationDashboard from './components/AIOptimizationDashboard';
import { LanguageProvider, useLanguage } from './contexts/LanguageContext';
import { t, type Language } from './i18n/translations';
// import { apiDiagnostics } from './utils/apiDiagnostics'; // 已移除，问题已修复
import type {
  SystemStatus,
  AccountInfo,
  Position,
...
type Page = 'competition' | 'trader' | 'environment' | 'api-config' | 'env-status' | 'risk-control-v2' | 'frequency-manager' | 'ai-optimization';
...
function App() {
  ...
  const getInitialPage = (): Page => {
    ...
    if (hash === 'environment') return 'environment';
    if (hash === 'api-config') return 'api-config';
    if (hash === 'env-status') return 'env-status';
    if (hash === 'risk-control-v2') return 'risk-control-v2';
    if (hash === 'frequency-manager') return 'frequency-manager';
    if (hash === 'ai-optimization') return 'ai-optimization';
    return 'competition';
  };
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      } else if (hash === 'env-status') {
        setCurrentPage('env-status');
      } else if (hash === 'risk-control-v2') {
        setCurrentPage('risk-control-v2');
      } else if (hash === 'frequency-manager') {
        setCurrentPage('frequency-manager');
      } else if (hash === 'ai-optimization') {
        setCurrentPage('ai-optimization');
  ...
  const handleConfigSave = (config: any) => {
    ...
    // 这里可以添加保存到后端或本地存储的逻辑
    // 例如：localStorage.setItem('nofx-config', JSON.stringify(config));
  };
    ...
    <div className="min-h-screen" style={{ background: '#0B0E11', color: '#EAECEF' }}>
        ...
        <div className="max-w-[1920px] mx-auto px-3 sm:px-6 py-3 sm:py-4">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-lg sm:text-xl" style={{ background: 'linear-gradient(135deg, #6B73FF 0%, #9F7AEA 100%)' }}>
                😴
              </div>
              <div>
            ...
            <div className="flex items-center gap-2 flex-wrap md:flex-nowrap">
              {/* Diagnostics Button - 已禁用，问题已修复 */}
              <button
                onClick={async () => {
                  console.log('🔍 诊断功能已禁用，数据源问题已修复');
                  // await apiDiagnostics.runFullDiagnostics(); // 已禁用
                }}
              ...
              {/* Page Toggle */}
              <div className="flex gap-0.5 sm:gap-1 rounded p-0.5 sm:p-1 flex-wrap" style={{ background: '#1E2329' }}>
                <button
                  onClick={() => navigateToPage('competition')}
                  className="px-2 sm:px-3 py-1.5 sm:py-2 rounded text-xs sm:text-sm font-semibold transition-all"
                ...
                <button
                  onClick={() => navigateToPage('environment')}
                  className="px-2 sm:px-3 py-1.5 sm:py-2 rounded text-xs sm:text-sm font-semibold transition-all"
                  style={currentPage === 'environment'
                    ? { background: '#F0B90B', color: '#000' }
                    : { background: 'transparent', color: '#848E9C' }
                  }
                >
                ...
                <button
                  onClick={() => navigateToPage('api-config')}
                  className="px-2 sm:px-3 py-1.5 sm:py-2 rounded text-xs sm:text-sm font-semibold transition-all"
                  style={currentPage === 'api-config'
                    ? { background: '#F0B90B', color: '#000' }
                    : { background: 'transparent', color: '#848E9C' }
                  }
                >
                ...
                <button
                  onClick={() => navigateToPage('env-status')}
                  className="px-2 sm:px-3 py-1.5 sm:py-2 rounded text-xs sm:text-sm font-semibold transition-all"
                  style={currentPage === 'env-status'
                    ? { background: '#F0B90B', color: '#000' }
                    : { background: 'transparent', color: '#848E9C' }
                  }
                >
                ...
                <button
                  onClick={() => navigateToPage('risk-control-v2')}
                  className="px-2 sm:px-3 py-1.5 sm:py-2 rounded text-xs sm:text-sm font-semibold transition-all"
                  style={currentPage === 'risk-control-v2'
                    ? { background: '#F0B90B', color: '#000' }
                    : { background: 'transparent', color: '#848E9C' }
                  }
                >
                ...
                <button
                  onClick={() => navigateToPage('frequency-manager')}
                  className="px-2 sm:px-3 py-1.5 sm:py-2 rounded text-xs sm:text-sm font-semibold transition-all"
                  style={currentPage === 'frequency-manager'
                    ? { background: '#F0B90B', color: '#000' }
                    : { background: 'transparent', color: '#848E9C' }
                  }
                >
                ...
                <button
                  onClick={() => navigateToPage('ai-optimization')}
                  className="px-2 sm:px-3 py-1.5 sm:py-2 rounded text-xs sm:text-sm font-semibold transition-all"
                  style={currentPage === 'ai-optimization'
                    ? { background: '#F0B90B', color: '#000' }
                    : { background: 'transparent', color: '#848E9C' }
                  }
                >
      ...
      <main className="max-w-[1920px] mx-auto px-6 py-6">
        ...
        {currentPage === 'environment' && (
          <div style={{ background: '#FFFFFF', minHeight: '80vh', borderRadius: '8px' }}>
            <EnvironmentSwitcher />
          </div>
        )}
        {currentPage === 'api-config' && (
          <div style={{ background: '#FFFFFF', minHeight: '80vh', borderRadius: '8px' }}>
            <ApiConfigPage onConfigUpdate={() => {
              // 配置更新后的回调，可以刷新其他组件的状态
              console.log('API配置已更新');
            }} />
          </div>
        )}
        {currentPage === 'env-status' && (
          <div style={{ background: '#FFFFFF', minHeight: '80vh', borderRadius: '8px' }}>
            <EnvironmentStatusPage />
          </div>
        )}
        {currentPage === 'risk-control-v2' && (
          <div style={{ background: '#FFFFFF', minHeight: '80vh', borderRadius: '8px' }}>
            <RiskControlV2Page />
          </div>
        )}
        {currentPage === 'frequency-manager' && (
          <div style={{ background: '#FFFFFF', minHeight: '80vh', borderRadius: '8px', padding: '24px' }}>
            <FrequencyManager traderId={selectedTraderId} />
          </div>
        )}
        {currentPage === 'ai-optimization' && <AIOptimizationDashboard />}
      </main>
    ...
    <div>
      ...
      <div className="mb-6 rounded p-6 animate-scale-in" style={{ background: 'linear-gradient(135deg, rgba(240, 185, 11, 0.15) 0%, rgba(252, 213, 53, 0.05) 100%)', border: '1px solid rgba(240, 185, 11, 0.2)', boxShadow: '0 0 30px rgba(240, 185, 11, 0.15)' }}>
        <h2 className="text-2xl font-bold mb-3 flex items-center gap-2" style={{ color: '#EAECEF' }}>
          <span className="w-10 h-10 rounded-full flex items-center justify-center text-xl" style={{ background: 'linear-gradient(135deg, #6B73FF 0%, #9F7AEA 100%)' }}>
            😴
          </span>
        ...
        {positions && positions.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {positions.map((pos, i) => (
                  <tr key={`${pos.symbol}-${pos.side}-${i}`} className="border-b border-gray-800 last:border-0">
          ...
          <div className="space-y-4 overflow-y-auto pr-2" style={{ maxHeight: 'calc(100vh - 280px)' }}>
            {decisions && decisions.length > 0 ? (
              decisions.map((decision, i) => (
                <DecisionCard key={`${decision.timestamp}-${i}`} decision={decision} language={language} />
              ))
            ) : (
      ...
      {decision.decisions && decision.decisions.length > 0 && (
        <div className="space-y-2 mb-3">
          {decision.decisions.map((action, j) => (
            <div key={`${action.symbol}-${action.action}-${j}`} className="flex items-center gap-2 text-sm rounded px-3 py-2" style={{ background: '#0B0E11' }}>
