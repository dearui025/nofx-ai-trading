...
export function EquityChart({ traderId }: EquityChartProps) {
  ...
  // 过滤掉无效数据：total_equity或total_value为0或小于1的数据点（API失败导致）
  const validHistory = history?.filter(point => (point.total_equity || point.total_value) > 1) || [];
  ...
  const initialBalance = validHistory[0]?.total_equity
    || validHistory[0]?.total_value
    || account?.total_equity
    || 100;  // 默认值改为100，与常见配置一致

  // 转换数据格式
  const chartData = displayHistory.map((point) => {
    const currentEquity = point.total_equity || point.total_value;
    ...
    return {
      ...
      value: displayMode === 'dollar' ? currentEquity : parseFloat(pnlPct),
