...
export function CompetitionPage() {
  ...
  const sortedTraders = [...competition.traders].sort(
    (a, b) => (b.daily_pnl_pct || 0) - (a.daily_pnl_pct || 0)
  );
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl" style={{
            ...
            boxShadow: '0 4px 14px rgba(107, 115, 255, 0.4)'
          }}>
            😴
          </div>
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2" style={{ color: '#EAECEF' }}>
              ...
              <span className="text-xs font-normal px-2 py-1 rounded" style={{ background: 'rgba(240, 185, 11, 0.15)', color: '#F0B90B' }}>
                {competition.traders.filter(t => t.status === 'active').length} {t('traders', language)}
              </span>
            </h1>
        ...
        <div className="text-right">
          ...
          <div className="text-lg font-bold" style={{ color: '#F0B90B' }}>{leader?.trader_name || 'N/A'}</div>
          <div className="text-sm font-semibold" style={{ color: (leader?.daily_pnl || 0) >= 0 ? '#0ECB81' : '#F6465D' }}>
            {(leader?.daily_pnl || 0) >= 0 ? '+' : ''}{(leader?.daily_pnl_pct || 0).toFixed(2)}%
          </div>
        </div>
      </div>

      {/* Three Column Layout: Chart + Leaderboard + Market Data */}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs mono font-semibold" style={{ color: traderColor }}>
                          {trader.ai_model?.toUpperCase() || 'AI'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-sm font-bold mono" style={{ color: '#EAECEF' }}>
                          {(trader.total_equity || 0).toFixed(2)}
                        </div>
                      </div>
                      ...
                      <div className="text-right min-w-[90px]">
                        ...
                        <div
                          ...
                          style={{ color: (trader.daily_pnl || 0) >= 0 ? '#0ECB81' : '#F6465D' }}
                        >
                          {(trader.daily_pnl || 0) >= 0 ? '+' : ''}
                          {(trader.daily_pnl_pct || 0).toFixed(2)}%
                        </div>
                        <div className="text-xs mono" style={{ color: '#848E9C' }}>
                          {(trader.daily_pnl || 0) >= 0 ? '+' : ''}{(trader.daily_pnl || 0).toFixed(2)}
                        </div>
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs" style={{ color: '#848E9C' }}>
                          {(trader.unrealized_pnl || 0).toFixed(1)}
                        </div>
                      </div>
        ...
        <div className="binance-card p-5 animate-slide-in" style={{ animationDelay: '0.3s' }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            {sortedTraders.map((trader, index) => {
              ...
              const gap = (trader.daily_pnl_pct || 0) - (opponent.daily_pnl_pct || 0);
              ...
              return (
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-2xl font-bold mono mb-1" style={{ color: (trader.daily_pnl || 0) >= 0 ? '#0ECB81' : '#F6465D' }}>
                      {(trader.daily_pnl || 0) >= 0 ? '+' : ''}{(trader.daily_pnl_pct || 0).toFixed(2)}%
                    </div>
                    {isWinning && gap > 0 && (
