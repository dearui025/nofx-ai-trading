...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["text/html", "text/css", "application/javascript", "image/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for nofx-frontend" ON storage.objects FOR SELECT USING (bucket_id = 'nofx-frontend');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for nofx-frontend" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'nofx-frontend');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for nofx-frontend" ON storage.objects FOR UPDATE USING (bucket_id = 'nofx-frontend');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for nofx-frontend" ON storage.objects FOR DELETE USING (bucket_id = 'nofx-frontend');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'nofx-frontend',
        public: true,
        allowed_mime_types: ["text/html", "text/css", "application/javascript", "image/*"],
