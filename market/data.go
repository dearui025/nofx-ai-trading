...
// getBinanceBaseURL 获取当前环境的币安API基础URL
func getBinanceBaseURL() string {
	// 尝试从环境配置获取URL
	envManager, err := config.NewEnvironmentManager("config/environments.json")
	if err == nil {
		currentEnv := envManager.GetCurrentEnvironment()
		if env, err := envManager.GetEnvironment(currentEnv); err == nil && env.BinanceBaseURL != "" {
			return env.BinanceBaseURL
		}
	}
	
	// 默认使用真实环境的API URL（公开API不需要密钥）
...
func getHTTPClient() *http.Client {
	...
	if err == nil {
		currentEnv := envManager.GetCurrentEnvironment()
		if env, err := envManager.GetEnvironment(currentEnv); err == nil {
			// 检查是否配置了代理
			proxyURL := env.ProxyURL
			if proxyURL == "" {
				// 如果环境配置中没有代理，使用默认的Clash代理
		...
		// 如果无法加载环境配置，直接使用默认的Clash代理
		if proxy, err := url.Parse("http://127.0.0.1:7890"); err == nil {
			transport := &http.Transport{
				Proxy: http.ProxyURL(proxy),
			}
			client.Transport = transport
...
func Get(symbol string) (*Data, error) {
	...
	// 获取1天K线数据 (最近30个，用于计算24小时变化)
	klines1d, err := getKlines(symbol, "1d", 30)
	if err != nil {
	...
	if len(klines1d) >= 2 {
		...
		// 获取最新一天的成交量
		volume24h = klines1d[len(klines1d)-1].Volume
	}
...
func calculateIntradaySeries(klines []Kline) *IntradayData {
	...
	// 获取最近30个数据点，确保有足够的数据用于市场状态分析（至少20个）
	start := len(klines) - 30
	if start < 0 {
		start = 0
	}
